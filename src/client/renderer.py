...
from core.entities import Food, Snake, StickyGoo
...
class Renderer:
    ...
    def desenhar_mundo(self, world: object) -> None:
        ...
        for goo in world.sticky_goos:
            self._desenhar_gosma(goo)
    ...
    def desenhar_hud(self, world: object) -> None:
        ...
        for pid in range(1, 5):
            ...
            if snake:
                sticky = snake.sticky_charges
                active = snake.sticky_active_timer
            ...
            label = self._fonte.render(
                f"J{pid} {pontos:04d} [{status}]{extra}",
                True,
                C.PLAYER_COLORS[pid],
            )
    ...
    def desenhar_menu(self, status_joy: dict[int, str | None]) -> None:
        ...
        self._tela.blit(
            lbl_start,
            (cx - lbl_start.get_width() // 2, rodape_y + 37),
        )
    ...
    def _desenhar_painel(
        ...
        tec = self._fonte_pequena.render(
            "TECLADO:",
            True,
            (140, 140, 140),
        )
    ...
    def _desenhar_gosma(self, goo: StickyGoo) -> None:
        col, row = goo.pos

        rect = pg.Rect(
            col * C.CELL + _MARGEM_GOSMA,
            row * C.CELL + _MARGEM_GOSMA,
            C.CELL - _MARGEM_GOSMA * 2,
            C.CELL - _MARGEM_GOSMA * 2,
        )

        pg.draw.ellipse(self._tela, C.STICKY_COLOR, rect)
    ...
    def _desenhar_comida(self, food: Food) -> None:
        ...
        cor = (
            C.STICKY_ORB_COLOR
            if getattr(food, "kind", "normal") == "sticky"
            else C.FOOD_COLOR
        )

        pg.draw.ellipse(self._tela, cor, rect)